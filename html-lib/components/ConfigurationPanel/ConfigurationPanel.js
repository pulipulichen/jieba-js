module.exports = {
  ...
  methods: {
    initExample () {
      this.config.state.configUserDictionaryExample = this.config.sesssion.configUserDictionary
      this.config.state.configWordRemapExample = thiss.config.sesssion.configWordRemap
      this.config.state.configStopWordsExample = thiss.config.sesssion.configStopWords
    },
    downloadConfiguration() {
      ...
      saveAs(new Blob([this.utils.Sheet.s2ab(wbout)], {type: "application/octet-stream"}), filename);
    },
    loadFullStopWords() {
