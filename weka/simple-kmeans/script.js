...
var _draw_stat_table = function (_result) {
	...
	// -------------------------
	// 先畫開頭
	_thead.append('<th>Attributes</th>');
	_thead.append('<th class="fulldata">Full Data and Avg.</th>');
	for (var _i = 0; _i < _cluster_data.length; _i++) {
		_thead.append('<th>Cluster ' + _i + '</th>');
	}
	...
	_count_tr.append('<th>Count</th>');
	_count_tr.append('<td title="Full Data, count" class="fulldata count">' 
		+ _full_count + '<br />(Avg: ' + _float_to_fixed((_full_count/_cluster_count.length), _to_fixed) + ')'
		+ '</td>');
	...
	for (var _a = 1; _a < _attr_list.length - 1; _a++) {
		...
		var _avg_tr = $('<tr class="avg-tr"></tr>').appendTo(_tbody);
		var _stddev_tr = $('<tr class="std-tr"></tr>').appendTo(_tbody);
...
var _draw_stat_abs_table = function () {
	...
	_thead_tr.find("th:first").html("Cluster");
